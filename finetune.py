...
class VPTDeep(nn.Module):
    def __init__(self, n_classes, encoder_name, prompt_length=10):
        ...
        num_layers = 12
        hidden_dim = 768
        self.prompts = nn.Parameter(torch.zeros(1, num_layers, prompt_length, hidden_dim))
        bound = (6 / (hidden_dim + prompt_length)) ** 0.5
    ...
    def forward(self, x):
        prompts = self.prompts.expand(x.size(0), -1, -1, -1)
        x = self.vit_b(x, prompts=prompts)
